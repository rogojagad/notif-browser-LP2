	...
	$.ajax({
		...
		success: function (data) {
			...
			if(data.now[0].jumlah > 0){
				...
				sendMessage("tidak-now", 'container-now', message);
			}
...
function sendMessage(bool, target, kegiatan) {
	...
	if (bool === "ada-next") {
		var info_kegiatan = "<p style='text-align: center; margin-top: 3em;'><strong>"+kegiatan.nama+"</strong></p>";
		info_kegiatan += "<p style='text-align: center'>"+kegiatan.mulai+" sampai "+kegiatan.selesai+"</p>";
	}
	else if(bool === "tidak-next"){
	...
	else if(bool === "tidak-now"){
		var info_kegiatan = "<p style='text-align:center;'>Tidak ada, let's have fun</p>";
	}
...
var endpoint;
getEndPoint();
setInterval(loadKegiatan, 600000);
